...
This application implements comprehensive security hardening based on OWASP Top 10 best practices:

- **Secure File Uploads**: Server-side validation with magic byte checking, file size limits, and image normalization
- **Input Sanitization**: Zod-based validation with control character removal and Unicode normalization
...
- **Authentication Scaffolding**: Demo token-based auth (placeholder for production systems)
- **Path Traversal Protection**: UUID validation and safe file serving
...
- `DEMO_AUTH_TOKEN`: Server-side authentication token
- `NEXT_PUBLIC_DEMO_AUTH_TOKEN`: Client-side auth token (for demo only)
- `NEXT_PUBLIC_SHOW_ADMIN`: Controls admin link visibility

⚠️ **Note**: The demo authentication is for workshop purposes only. For production, implement proper authentication using NextAuth.js, Clerk, or Auth0.
...
│   ├── api/             # API routes
│   │   └── uploads/     # Secure file upload endpoints
│   ├── gallery/         # Gallery page
│   ├── upload/          # Upload page
│   └── admin/           # Admin dashboard
├── components/          # Reusable React components
│   ├── ui/              # UI components (layout, cards, etc.)
...
For security best practices and production recommendations, see [SECURITY.md](SECURITY.md).